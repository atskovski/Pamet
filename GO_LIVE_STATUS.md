# Pamet 1.6.3 — Go-Live Status at a Glance

Last reviewed: 2026-09-03  
Source of truth: Pamet 1.6.3 production runtime plus exact-commit CI/environment evidence.  
...
| Dark-mode visual system | **1.6.3 release baseline; deployed verification required** | Unified dark surfaces, readable text hierarchy and current release identity are enforced in source; PWA cache rotation prevents an old visual bundle from remaining indefinitely. |
...
| PWA release delivery | **1.6.3 cache rotation enforced** | Worker registration `sw.js?v=1630`, shell cache `pamet-shell-v163-1`, CSS/JS shell query token `v=163`, and version checks are release-specific. Future releases fail CI if these values are not rotated together. |
...
| Mobile contract | **1.6.3 backend contract; 1.5.1 minimum compatible baseline** | `contracts/mobile-api.json` publishes backend 1.6.3 while retaining minimum backend 1.5.1 so the merged iOS/Android 1.5.1 baselines remain compatible. |
