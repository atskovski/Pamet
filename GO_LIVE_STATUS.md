...
Source of truth: Pamet 1.6.1 production runtime plus exact-commit CI/environment evidence.  
...
| Dark-mode visual system | **1.6.1 hardened; deployed verification required** | Unified dark surfaces, readable text hierarchy and current release identity are enforced in source; PWA cache rotation prevents an old visual bundle from remaining indefinitely. |
| Data portability | **Implemented in product** | Settings exposes a local JSON export using the existing full-store export contract. Export generation stays in the browser and does not upload the journal to a new export service. A clinician-oriented PDF package remains future product work. |
