...
(function(global){
  "use strict";
  const USER_KEY="pamet_user_v1",SESSION_KEY="pamet_session_v2",LEGACY_SESSION_KEY="pamet_session_v1",ROUNDS=600000,MIN_PASSWORD_LENGTH=10;
  ...
  async function bootstrapAccount(){const c=Auth.getBackendCredential();if(!c)return;const{deviceKey,...profile}=c;try{await fetch("/api/account/bootstrap",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${deviceKey}`},body:JSON.stringify({...profile,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||"UTC"})})}catch{/* optional service: local journal still works */}}
  migrateUser();
  const Auth={
    isSecure:!!subtle(),
    ...
    async login(email,password){requireCrypto();const u=loadUser();if(!u)throw new Error("No account found on this device.");if((email||"").trim().toLowerCase()!==u.email)throw new Error("Email not recognized.");if(!(await verify(password,u)))throw new Error("Incorrect password.");if(u.algo!=="pbkdf2"||(u.iterations||0)<ROUNDS){const salt=randomHex(16),d=await derive(password,salt);u.salt=salt;u.hash=d.hash;u.iterations=d.iterations;u.algo=d.algo;saveUser(u)}this.startSession();emit("pamet:login",this.getUser());setTimeout(bootstrapAccount,0);return u},
    ...
    async changePassword(oldPassword,newPassword){requireCrypto();const u=loadUser();if(!u)throw new Error("No account found.");if(String(newPassword||"").length<MIN_PASSWORD_LENGTH)throw new Error(`New password must be at least ${MIN_PASSWORD_LENGTH} characters.`);if(!(await verify(oldPassword,u)))throw new Error("Current password is incorrect.");const salt=randomHex(16),d=await derive(newPassword,salt);u.salt=salt;u.hash=d.hash;u.iterations=d.iterations;u.algo=d.algo;saveUser(u)}
  };
