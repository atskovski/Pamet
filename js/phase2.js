/* Pamet v1.2.0 — profiles, appointment preparation, health-history comparisons, and advanced sharing. */
(function () {
  "use strict";
  ...
  async function appointmentPrep() {
    ...
    let appointments = []; try { appointments = (await api("/api/appointments")).appointments || []; } catch (error) { return toast(error.message, true); }
    const root = modal(`<div class="pamet-modal-head"><div><h2 class="pamet-modal-title">Appointment workspace</h2><p class="pamet-modal-sub">Plan the visit, save your questions, and create a focused discussion guide.</p></div><button class="pamet-close" data-close aria-label="Close">×</button></div><form id="appointmentForm" class="pamet-form"><label>Clinician or practice<input id="appointmentClinician" maxlength="120" required></label><label>Date and time<input id="appointmentStarts" type="datetime-local" required></label><label>Reason for visit<input id="appointmentReason" maxlength="500"></label><label>Top concerns, one per line<textarea id="appointmentConcerns" maxlength="2000"></textarea></label><label>Questions, one per line<textarea id="appointmentQuestions" maxlength="2000">${questions.map(esc).join("\n")}</textarea></label><label>Reminder<select id="appointmentReminder"><option value="60">1 hour before</option><option value="1440" selected>1 day before</option><option value="10080">1 week before</option></select></label><button class="btn btn-primary btn-block">Save appointment</button></form><section class="phase2-prep"><h3>Upcoming and saved visits</h3><div id="appointmentList">${appointments.length ? appointments.map((item) => `<div class="phase2-profile-row"><div><strong>${esc(item.clinician)}</strong><small>${esc(new Date(item.startsAt).toLocaleString())} · ${esc(item.reason || "General visit")}</small></div><button class="pamet-text-button" data-remove-appointment="${esc(item.id)}">Remove</button></div>`).join("") : "<p>No appointments saved yet.</p>"}</div><h3>Recorded changes to discuss</h3>${data.names.length ? `<ul>${data.names.slice(0, 4).map((name) => `<li><strong>${esc(name)}</strong>: ${data.currentCounts[name] || 0} recent vs ${data.previousCounts[name] || 0} previous logged days</li>`).join("")}</ul>` : `<p>Keep logging to build a before-and-after comparison.</p>`}</section><p class="pamet-reassurance">Pamet organizes what you recorded. It does not recommend diagnoses, tests, or treatments.</p>`);
    const lines = (value) => String(value || "").split(/\n+/).map((item) => item.trim()).filter(Boolean).slice(0, 10);
    root.querySelector("#appointmentForm").addEventListener("submit", async (event) => { event.preventDefault(); try { await api("/api/appointments", { method: "POST", body: JSON.stringify({ profileId: S.activeProfile.id, clinician: root.querySelector("#appointmentClinician").value, startsAt: new Date(root.querySelector("#appointmentStarts").value).toISOString(), reason: root.querySelector("#appointmentReason").value, concerns: lines(root.querySelector("#appointmentConcerns").value), questions: lines(root.querySelector("#appointmentQuestions").value), reminderMinutes: +root.querySelector("#appointmentReminder").value }) }); root.innerHTML = ""; toast("Appointment saved."); appointmentPrep(); } catch (error) { toast(error.message, true); } });
    root.querySelectorAll("[data-remove-appointment]").forEach((button) => button.addEventListener("click", async () => { try { await api(`/api/appointments/${encodeURIComponent(button.dataset.removeAppointment)}`, { method: "DELETE" }); button.closest(".phase2-profile-row").remove(); toast("Appointment removed."); } catch (error) { toast(error.message, true); } }));
  }
  ...
  function injectSettings() {
    ...
    const footer = $(".footer-line"); if (footer) footer.textContent = "Pamet v1.2.0 · Your health history, finally useful.";
