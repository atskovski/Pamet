/* Pamet interaction fixes — resilient Insights controls and live plan-aware logging feedback. */
(() => {
  'use strict';
  ...
  const planCategories = Object.freeze({
    symptoms: Object.freeze({ selector: '#symptomGrid .sym-btn.selected', customKey: 'customSymptoms', plural: 'symptoms', emptyLabel: 'symptoms' }),
    moods: Object.freeze({ selector: '#moodFlow .chip.selected', customKey: 'customMoods', plural: 'moods', emptyLabel: 'moods' }),
    activities: Object.freeze({ selector: '#activityFlow .chip.selected', customKey: 'customActivities', plural: 'activities', emptyLabel: 'activities' }),
    meds: Object.freeze({ selector: '#medFlow .chip.selected', customKey: 'customMeds', plural: 'medications', emptyLabel: 'medications' })
  });
  ...
  function emptyState(info, archived) {
    if (state.showArchived) {
      return `<section class="insights-empty"><span data-pamet-icon="archive"></span><h3>No archived observations</h3><p>Archiving removes an observation from the active Insights workspace only. Your journal entries stay unchanged.</p></section>`;
    }
  ...
  function renderInsights() {
    ...
    /* Keep the previous Insights renderer dormant. The application-level legacy bridge
     * can still populate its hidden compatibility targets without replacing this view. */
  ...
  function flashStatus(message) {
    ...
    window.setTimeout(() => {
      ...
      state.status = '';
      const status = $('#screen-patterns .insights-action-status');
      status?.remove();
    }, 4200);
  }
