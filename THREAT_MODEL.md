...
Pamet v1.0.5 is local-first, not end-to-end encrypted. Journal entries and profile data are stored as readable browser local storage for the signed-in browser profile. The device-local password protects access through Pamet's interface, but it does not encrypt the underlying journal records.
