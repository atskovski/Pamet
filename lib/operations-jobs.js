...
async function runWeeklyDigest({ connection, sendMail, appBaseUrl, batchSize = 250, log = () => {} }) {
  ...
  for await (const users of iterateById({
    ...
    for (const user of users) {
      attempted += 1;
      const body = `<h1 style="font-size:22px">Your weekly Pamet summary is ready.</h1><p>Open Pamet to review your weekly summary.</p><p>For privacy, this email does not include symptoms, medications, sleep, mood, notes, or other health details.</p><p><a href="${html(appBaseUrl)}">Open Pamet</a></p>`;
      try {
