...
async function breachedPassword(password, fetchImpl = globalThis.fetch) {
  if (typeof fetchImpl !== 'function') throw new Error('Password breach lookup is unavailable.');
  const digest = crypto.createHash('sha1').update(String(password), 'utf8').digest('hex').toUpperCase();
  const prefix = digest.slice(0, 5); const suffix = digest.slice(5);
  const response = await fetchImpl(`https://api.pwnedpasswords.com/range/${prefix}`, {
    headers: { 'Add-Padding': 'true', 'User-Agent': 'Pamet/1.2 password-safety-check' },
    signal: AbortSignal.timeout(5000)
  });
  if (!response.ok) throw new Error(`Password breach lookup failed with HTTP ${response.status}.`);
  const body = await response.text();
  return body.split(/\r?\n/).some((line) => line.split(':', 1)[0].trim().toUpperCase() === suffix);
}

module.exports = { totpSecret, totp, verifyTotp, seal, open, breachedPassword };
