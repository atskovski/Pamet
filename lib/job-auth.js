...
function workflowAllowed(value, allowed) {
  if (!value || !allowed.length) return false;
  return allowed.some((workflow) => value === `${REPOSITORY}/.github/workflows/${workflow}@${MAIN_REF}`);
}
async function fetchJwks(fetcher = fetch) {
...
async function verifyGitHubActionsToken(token, {
  ...
  let keys;
  try { keys = await fetchJwks(fetcher); }
  ...
  if (claims.repository !== REPOSITORY || claims.ref !== MAIN_REF) return { ok: false, reason: 'repository' };
  ...
  if (!workflowAllowed(workflowRef, allowedWorkflows)) return { ok: false, reason: 'workflow' };

  return { ok: true, source: 'github-oidc', claims: { event_name: claims.event_name, workflow_ref: workflowRef, run_id: claims.run_id || null } };
}
