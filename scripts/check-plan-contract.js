...
check(comparison.includes('asset-manifest.json') && comparison.includes('?release='), 'Deferred plan matrix must bypass stale service-worker cache on each deployment.');
...
check(managementLoader.includes('asset-manifest.json') && managementLoader.includes('?release='), 'Deferred plan management must bypass stale service-worker cache on each deployment.');
