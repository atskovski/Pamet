...
check(main.includes("navigator.serviceWorker.register('sw.js?v=1300')"), 'PWA registration must reference the current release worker.');
check(main.includes('version-update-v1.3.0.js'), 'The production bundle must load the safe new-version update flow.');
...
check(worker.includes(`Pamet v${expected}`) && worker.includes('pamet-shell-v130-0'), 'Service worker cache/version must match the release.');
