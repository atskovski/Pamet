...
transformFile('app.js', (input) => {
  ...
  source = replaceRequired(source,
    ...
    'report bullet style');
  source = replaceRequired(source,
    '<tr><td>${esc(k)}</td><td style="text-align:right;font-weight:600">${esc(v)}</td></tr>',
    '<tr><td>${esc(k)}</td><td class="report-value">${esc(v)}</td></tr>',
    'print report value style');
  source = replaceRequired(source,
...
const browserSources = fs.readdirSync(tempJs).filter((name) => name.endsWith('.js'));
for (const name of browserSources) {
  const source = fs.readFileSync(path.join(tempJs, name), 'utf8');
  if (/\bstyle\s*=\s*["']/.test(source)) throw new Error(`Strict CSP build still contains a style attribute in js/${name}`);
  if (/\.style\.(?:setProperty|cssText|display|overflow|width|height|color|background)/.test(source)) throw new Error(`Strict CSP build still contains presentation CSSOM mutation in js/${name}`);
}
