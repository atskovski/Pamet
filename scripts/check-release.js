...
check(main.includes("const PAMET_VERSION = '1.6.0'")&&main.includes('./performance.js')&&main.includes('./insights.js')&&main.includes('./experience.js')&&main.includes('./icons.js')&&main.includes('./version-update.js'),'1.6.0 feature-owned runtime modules must be explicit.');
...
check(main.includes("navigator.serviceWorker.register('sw.js?v=1600')"),'Service worker must register from the external production bundle under strict CSP.');
check(worker.includes('pamet-shell-v160-1')&&worker.includes('dist/pamet.min.js?v=160')&&worker.includes('ignoreSearch:true'),'1.6.0 service worker must use fast version-tolerant shell caching.');
...
console.log('Pamet v1.6.0 release checks passed.');
