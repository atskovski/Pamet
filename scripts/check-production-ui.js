...
const switchUi = fs.readFileSync('js/account-switch-v1.2.0.js', 'utf8');
const localEncryption = fs.readFileSync('js/local-encryption-v1.2.0.js', 'utf8');
...
check(main.includes('account-switch-v1.2.0.js'), 'Production bundle must include safe account switching.');
check(main.includes('qr-v1.2.0.js') && main.includes('security-v1.1.0.js'), 'Authenticator QR/security modules must ship together.');
check(!main.includes('local-encryption-v1.2.0.js'), 'Local encryption must remain disabled until independent review approves migration enablement.');
check(cssMain.includes('mobile-v1.2.0.css'), 'Final mobile viewport overrides must load last.');
...
check(secureServer.includes("script-src-attr 'none'") && secureServer.includes('hardenedCsp'), 'Executable inline script CSP hardening must remain enabled.');
