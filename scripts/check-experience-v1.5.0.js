...
check(main.includes('./icons.js') && authenticated.includes('./insights.js') && authenticated.includes('./experience.js') && !main.includes('import "./insights.js"'), 'Feature-owned product-system runtime layers must remain bundled while paid/heavy Insights and experience code stay deferred.');
...
check(darkMode.includes('.completeness-card') && darkMode.includes('.insights-empty') && darkMode.includes('--text-primary: #F2F5F4'), 'Dark-mode Insights surfaces and readable foreground hierarchy must remain enforced.');
