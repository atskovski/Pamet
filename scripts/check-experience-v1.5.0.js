...
check(main.includes('./icons.js') && authenticated.includes('./insights.js') && authenticated.includes('./interaction-controller.js') && authenticated.includes('./experience.js') && !main.includes('import "./insights.js"'), 'Feature-owned product-system runtime layers must remain bundled while paid/heavy Insights and experience code stay deferred.');
...
check(authenticatedCss.includes('@import "./insights-overhaul.css";') && insightsCss.includes('.tracking-quality-card') && insightsCss.includes('.insights-window-kpis'), 'Redesigned Insights surfaces must remain in the deferred authenticated stylesheet.');
...
check(insights.includes('does not establish') && insights.includes('does not infer') && insightsController.includes('does not diagnose') && !/(?:\bcauses\b|\bcaused by\b|\bleads to\b|→)/i.test(insights + insightsController), 'Insights language must remain observational and non-causal.');
